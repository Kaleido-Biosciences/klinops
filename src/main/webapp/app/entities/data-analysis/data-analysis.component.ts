...
export class DataAnalysisComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.dataAnalysisService
      ...
      .subscribe(
        (res: HttpResponse<IDataAnalysis[]>) => this.paginateDataAnalyses(res.body, res.headers),
  ...
  trackId(index: number, item: IDataAnalysis) {
    return item.id;
  }
  ...
  protected paginateDataAnalyses(data: IDataAnalysis[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.dataAnalyses = data;
  }
