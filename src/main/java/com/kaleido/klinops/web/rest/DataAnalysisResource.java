...
public class DataAnalysisResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/data-analyses?query=:query} : search for the dataAnalysis corresponding
     * to the query.
     *
     * @param query the query of the dataAnalysis search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/data-analyses")
    public ResponseEntity<List<DataAnalysis>> searchDataAnalyses(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of DataAnalyses for query {}", query);
        Page<DataAnalysis> page = dataAnalysisService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
