...
public class ShipmentComponentResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/shipment-components?query=:query} : search for the shipmentComponent corresponding
     * to the query.
     *
     * @param query the query of the shipmentComponent search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/shipment-components")
    public ResponseEntity<List<ShipmentComponent>> searchShipmentComponents(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ShipmentComponents for query {}", query);
        Page<ShipmentComponent> page = shipmentComponentService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
