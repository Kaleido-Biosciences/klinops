...
public class LaboratoryResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/laboratories?query=:query} : search for the laboratory corresponding
     * to the query.
     *
     * @param query the query of the laboratory search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/laboratories")
    public ResponseEntity<List<Laboratory>> searchLaboratories(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Laboratories for query {}", query);
        Page<Laboratory> page = laboratoryService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
