...
public class StudyProductResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/study-products?query=:query} : search for the studyProduct corresponding
     * to the query.
     *
     * @param query the query of the studyProduct search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/study-products")
    public ResponseEntity<List<StudyProduct>> searchStudyProducts(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of StudyProducts for query {}", query);
        Page<StudyProduct> page = studyProductService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
