...
public class StudyEndPointResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/study-end-points?query=:query} : search for the studyEndPoint corresponding
     * to the query.
     *
     * @param query the query of the studyEndPoint search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/study-end-points")
    public ResponseEntity<List<StudyEndPoint>> searchStudyEndPoints(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of StudyEndPoints for query {}", query);
        Page<StudyEndPoint> page = studyEndPointService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
