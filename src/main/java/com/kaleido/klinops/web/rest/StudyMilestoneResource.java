...
public class StudyMilestoneResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/study-milestones?query=:query} : search for the studyMilestone corresponding
     * to the query.
     *
     * @param query the query of the studyMilestone search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/study-milestones")
    public ResponseEntity<List<StudyMilestone>> searchStudyMilestones(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of StudyMilestones for query {}", query);
        Page<StudyMilestone> page = studyMilestoneService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
