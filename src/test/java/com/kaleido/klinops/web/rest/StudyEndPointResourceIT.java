...
public class StudyEndPointResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.StudyEndPointSearchRepositoryMockConfiguration
     */
    @Autowired
    private StudyEndPointSearchRepository mockStudyEndPointSearchRepository;
    ...
    public void createStudyEndPoint() throws Exception {
        ...
        // Validate the StudyEndPoint in Elasticsearch
        verify(mockStudyEndPointSearchRepository, times(1)).save(testStudyEndPoint);
    }
    ...
    public void createStudyEndPointWithExistingId() throws Exception {
        ...
        // Validate the StudyEndPoint in Elasticsearch
        verify(mockStudyEndPointSearchRepository, times(0)).save(studyEndPoint);
    }
    ...
    public void updateStudyEndPoint() throws Exception {
        ...
        reset(mockStudyEndPointSearchRepository);
    ...
    public void deleteStudyEndPoint() throws Exception {
        ...
        // Validate the StudyEndPoint in Elasticsearch
        verify(mockStudyEndPointSearchRepository, times(1)).deleteById(studyEndPoint.getId());
    }

    @Test
    @Transactional
    public void searchStudyEndPoint() throws Exception {
        // Initialize the database
        studyEndPointService.save(studyEndPoint);
        when(mockStudyEndPointSearchRepository.search(queryStringQuery("id:" + studyEndPoint.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(studyEndPoint), PageRequest.of(0, 1), 1));
        // Search the studyEndPoint
        restStudyEndPointMockMvc.perform(get("/api/_search/study-end-points?query=id:" + studyEndPoint.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(studyEndPoint.getId().intValue())))
            .andExpect(jsonPath("$.[*].description").value(hasItem(DEFAULT_DESCRIPTION)))
