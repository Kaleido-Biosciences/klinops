...
public class BioAnalysisResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.BioAnalysisSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createBioAnalysis() throws Exception {
        ...
        // Validate the BioAnalysis in Elasticsearch
        verify(mockBioAnalysisSearchRepository, times(1)).save(testBioAnalysis);
    }
    ...
    public void createBioAnalysisWithExistingId() throws Exception {
        ...
        // Validate the BioAnalysis in Elasticsearch
        verify(mockBioAnalysisSearchRepository, times(0)).save(bioAnalysis);
    }
    ...
    public void updateBioAnalysis() throws Exception {
        ...
        reset(mockBioAnalysisSearchRepository);
    ...
    public void deleteBioAnalysis() throws Exception {
        ...
        // Validate the BioAnalysis in Elasticsearch
        verify(mockBioAnalysisSearchRepository, times(1)).deleteById(bioAnalysis.getId());
    }

    @Test
    @Transactional
    public void searchBioAnalysis() throws Exception {
        // Initialize the database
        bioAnalysisService.save(bioAnalysis);
        when(mockBioAnalysisSearchRepository.search(queryStringQuery("id:" + bioAnalysis.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(bioAnalysis), PageRequest.of(0, 1), 1));
        // Search the bioAnalysis
        restBioAnalysisMockMvc.perform(get("/api/_search/bio-analyses?query=id:" + bioAnalysis.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(bioAnalysis.getId().intValue())))
            .andExpect(jsonPath("$.[*].analyte").value(hasItem(DEFAULT_ANALYTE)))
