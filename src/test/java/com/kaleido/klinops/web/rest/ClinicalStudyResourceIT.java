...
public class ClinicalStudyResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.ClinicalStudySearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createClinicalStudy() throws Exception {
        ...
        // Validate the ClinicalStudy in Elasticsearch
        verify(mockClinicalStudySearchRepository, times(1)).save(testClinicalStudy);
    }
    ...
    public void createClinicalStudyWithExistingId() throws Exception {
        ...
        // Validate the ClinicalStudy in Elasticsearch
        verify(mockClinicalStudySearchRepository, times(0)).save(clinicalStudy);
    }
    ...
    public void updateClinicalStudy() throws Exception {
        ...
        reset(mockClinicalStudySearchRepository);
    ...
    public void deleteClinicalStudy() throws Exception {
        ...
        // Validate the ClinicalStudy in Elasticsearch
        verify(mockClinicalStudySearchRepository, times(1)).deleteById(clinicalStudy.getId());
    }

    @Test
    @Transactional
    public void searchClinicalStudy() throws Exception {
        // Initialize the database
        clinicalStudyService.save(clinicalStudy);
        when(mockClinicalStudySearchRepository.search(queryStringQuery("id:" + clinicalStudy.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(clinicalStudy), PageRequest.of(0, 1), 1));
        // Search the clinicalStudy
        restClinicalStudyMockMvc.perform(get("/api/_search/clinical-studies?query=id:" + clinicalStudy.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(clinicalStudy.getId().intValue())))
            .andExpect(jsonPath("$.[*].studyIdentifier").value(hasItem(DEFAULT_STUDY_IDENTIFIER)))
