...
public class DataAnalysisResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.DataAnalysisSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createDataAnalysis() throws Exception {
        ...
        // Validate the DataAnalysis in Elasticsearch
        verify(mockDataAnalysisSearchRepository, times(1)).save(testDataAnalysis);
    }
    ...
    public void createDataAnalysisWithExistingId() throws Exception {
        ...
        // Validate the DataAnalysis in Elasticsearch
        verify(mockDataAnalysisSearchRepository, times(0)).save(dataAnalysis);
    }
    ...
    public void updateDataAnalysis() throws Exception {
        ...
        reset(mockDataAnalysisSearchRepository);
    ...
    public void deleteDataAnalysis() throws Exception {
        ...
        // Validate the DataAnalysis in Elasticsearch
        verify(mockDataAnalysisSearchRepository, times(1)).deleteById(dataAnalysis.getId());
    }

    @Test
    @Transactional
    public void searchDataAnalysis() throws Exception {
        // Initialize the database
        dataAnalysisService.save(dataAnalysis);
        when(mockDataAnalysisSearchRepository.search(queryStringQuery("id:" + dataAnalysis.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(dataAnalysis), PageRequest.of(0, 1), 1));
        // Search the dataAnalysis
        restDataAnalysisMockMvc.perform(get("/api/_search/data-analyses?query=id:" + dataAnalysis.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(dataAnalysis.getId().intValue())))
            .andExpect(jsonPath("$.[*].dataAnalysesType").value(hasItem(DEFAULT_DATA_ANALYSES_TYPE)))
