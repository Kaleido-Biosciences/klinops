...
public class LaboratoryResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.LaboratorySearchRepositoryMockConfiguration
     */
    @Autowired
    private LaboratorySearchRepository mockLaboratorySearchRepository;
    ...
    public void createLaboratory() throws Exception {
        ...
        // Validate the Laboratory in Elasticsearch
        verify(mockLaboratorySearchRepository, times(1)).save(testLaboratory);
    }
    ...
    public void createLaboratoryWithExistingId() throws Exception {
        ...
        // Validate the Laboratory in Elasticsearch
        verify(mockLaboratorySearchRepository, times(0)).save(laboratory);
    }
    ...
    public void updateLaboratory() throws Exception {
        ...
        reset(mockLaboratorySearchRepository);
    ...
    public void deleteLaboratory() throws Exception {
        ...
        // Validate the Laboratory in Elasticsearch
        verify(mockLaboratorySearchRepository, times(1)).deleteById(laboratory.getId());
    }

    @Test
    @Transactional
    public void searchLaboratory() throws Exception {
        // Initialize the database
        laboratoryService.save(laboratory);
        when(mockLaboratorySearchRepository.search(queryStringQuery("id:" + laboratory.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(laboratory), PageRequest.of(0, 1), 1));
        // Search the laboratory
        restLaboratoryMockMvc.perform(get("/api/_search/laboratories?query=id:" + laboratory.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(laboratory.getId().intValue())))
            .andExpect(jsonPath("$.[*].labName").value(hasItem(DEFAULT_LAB_NAME)))
