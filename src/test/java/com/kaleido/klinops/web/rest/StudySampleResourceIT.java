...
public class StudySampleResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.StudySampleSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createStudySample() throws Exception {
        ...
        // Validate the StudySample in Elasticsearch
        verify(mockStudySampleSearchRepository, times(1)).save(testStudySample);
    }
    ...
    public void createStudySampleWithExistingId() throws Exception {
        ...
        // Validate the StudySample in Elasticsearch
        verify(mockStudySampleSearchRepository, times(0)).save(studySample);
    }
    ...
    public void updateStudySample() throws Exception {
        ...
        reset(mockStudySampleSearchRepository);
    ...
    public void deleteStudySample() throws Exception {
        ...
        // Validate the StudySample in Elasticsearch
        verify(mockStudySampleSearchRepository, times(1)).deleteById(studySample.getId());
    }

    @Test
    @Transactional
    public void searchStudySample() throws Exception {
        // Initialize the database
        studySampleService.save(studySample);
        when(mockStudySampleSearchRepository.search(queryStringQuery("id:" + studySample.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(studySample), PageRequest.of(0, 1), 1));
        // Search the studySample
        restStudySampleMockMvc.perform(get("/api/_search/study-samples?query=id:" + studySample.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(studySample.getId().intValue())))
            .andExpect(jsonPath("$.[*].sampleType").value(hasItem(DEFAULT_SAMPLE_TYPE)))
