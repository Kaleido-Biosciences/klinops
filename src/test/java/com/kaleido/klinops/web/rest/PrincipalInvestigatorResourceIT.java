...
public class PrincipalInvestigatorResourceIT {
     ...
     *
     * @see com.kaleido.klinops.repository.search.PrincipalInvestigatorSearchRepositoryMockConfiguration
     */
    @Autowired
    private PrincipalInvestigatorSearchRepository mockPrincipalInvestigatorSearchRepository;
    ...
    public void createPrincipalInvestigator() throws Exception {
        ...
        // Validate the PrincipalInvestigator in Elasticsearch
        verify(mockPrincipalInvestigatorSearchRepository, times(1)).save(testPrincipalInvestigator);
    }
    ...
    public void createPrincipalInvestigatorWithExistingId() throws Exception {
        ...
        // Validate the PrincipalInvestigator in Elasticsearch
        verify(mockPrincipalInvestigatorSearchRepository, times(0)).save(principalInvestigator);
    }
    ...
    public void updatePrincipalInvestigator() throws Exception {
        ...
        reset(mockPrincipalInvestigatorSearchRepository);
    ...
    public void deletePrincipalInvestigator() throws Exception {
        ...
        // Validate the PrincipalInvestigator in Elasticsearch
        verify(mockPrincipalInvestigatorSearchRepository, times(1)).deleteById(principalInvestigator.getId());
    }

    @Test
    @Transactional
    public void searchPrincipalInvestigator() throws Exception {
        // Initialize the database
        principalInvestigatorService.save(principalInvestigator);
        when(mockPrincipalInvestigatorSearchRepository.search(queryStringQuery("id:" + principalInvestigator.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(principalInvestigator), PageRequest.of(0, 1), 1));
        // Search the principalInvestigator
        restPrincipalInvestigatorMockMvc.perform(get("/api/_search/principal-investigators?query=id:" + principalInvestigator.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(principalInvestigator.getId().intValue())))
            .andExpect(jsonPath("$.[*].investigatorName").value(hasItem(DEFAULT_INVESTIGATOR_NAME)))
